...
%OBa rate
y1_low = 5.381*10.^-10*t.^3 - 1.543*10.^-6*t.^2 +0.001343*t +0.9461;

%OCa rate
y2_low = 2.083*10.^-10*t.^3 - 6.61*10.^-7*t.^2 + 0.0007185*t + 1.119;

% We compute the bone volume percentage loss due to mm:
BVL_low = abs(-2*y1_low + 0.334*y2_low);

% integration of BMD loss with BV loss due to multiple myeloma:
TotalBMD_high = (((-69) * exp(lambda * t .* BVL_high) + 69) * -1 );
TotalBMD_2high = (((-69) * exp(lambda * t .* BVL_2high) + 69) * -1 );
TotalBMD_2low = (((-69) * exp(lambda * t .* BVL_2low) + 69) * -1 );
TotalBMD_low = (((-69) * exp(lambda * t .* BVL_low) + 69) * -1 );
...
% Plots for bmd loss with MM
plot(t,TotalBMD_low, 'k', 'LineWidth', 1.5);
hold on
plot(t,TotalBMD_2low, 'm', 'LineWidth', 1.5);
hold on
plot(t,TotalBMD_2high, 'g', 'LineWidth', 1.5);
hold on
plot(t,TotalBMD_high, 'b', 'LineWidth', 1.5);
hold on

% Addition of vertical line to indicate length of round trip mars mission
xline(1095, '--')
text(1095, max(BMD), 'Day 1095', 'VerticalAlignment', 'top', 'HorizontalAlignment', 'left');

legend('Pmm = 0.00/day', 'Pmm = 0.035/day', 'Pmm = 0.045/day', 'Pmm = 0.055/day', 'Pmm = 0.065/day')
lgd = legend;
lgd.FontSize = 12;
lgd.Title.String = 'Proliferation Rates';
title('BMD% Change with Microgravity and Multiple Myeloma Proliferation Rates')
xlabel("Time (days)")
